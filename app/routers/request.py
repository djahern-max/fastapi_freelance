...
def get_public_requests(
    ...
    try:
        developer_id = (
            current_user.id
            if current_user and current_user.user_type == UserType.developer
            else None
        )
        requests = crud_request.get_public_requests(
            db=db, skip=skip, limit=limit, developer_id=developer_id
        )
