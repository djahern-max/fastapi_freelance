...
def create_post(
    ...
    new_post = models.Post(**post.model_dump(), user_id=current_user.id, note_id=note_id)
    db.add(new_post)
    db.commit()
    db.refresh(new_post)
    return new_post
...
def create_comment(
    ...
    new_comment = models.Comment(content=comment.content, user_id=current_user.id, post_id=post_id)
    db.add(new_comment)
    db.commit()
