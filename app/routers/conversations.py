...
def get_conversation(
    ...
    recipient = db.query(models.User).filter(models.User.id == conversation.recipient_user_id).first()
...
def list_user_conversations(
    ...
    for conv in conversations:
        ...
        request = db.query(models.Request).filter(
            models.Request.id == conv.request_id
        ).first()
        ...
        starter = db.query(models.User).filter(
            models.User.id == conv.starter_user_id
        ).first()
        recipient = db.query(models.User).filter(
            models.User.id == conv.recipient_user_id
        ).first()
